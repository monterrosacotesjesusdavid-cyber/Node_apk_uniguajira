...
async function runSeed() {
  try {
    ...
    if (totalAdmin === 0) {
      ...
      await db.query('INSERT INTO admins (nombre, correo, password_hash) VALUES (?,?,?)',
    ...
    for (const [nombre, correo, cedula] of profesores) {
      ...
      if (existe === 0) {
        await db.query('INSERT INTO profesores (nombre, correo, cedula, password_hash) VALUES (?,?,?,?)',
    ...
    for (const [nombre, username, codigo] of estudiantes) {
      ...
      if (existe === 0) {
        ...
        await db.query('INSERT INTO estudiantes (nombre, username, codigo_hash) VALUES (?,?,?)',
    ...
    const horariosDef = [
      ...
      ['1001234567', 'Bloque 6 - Piso 1 - Salón 04', 'Cálculo Numérico',          'Sábado',    '09:00:00', '11:00:00'],
      ['1001234567', 'Bloque 6 - Piso 1 - Salón 03', 'Laboratorio de Física',     'Sábado',    '14:00:00', '16:00:00'],
      ['1001234567', 'Bloque 6 - Piso 1 - Salón 04', 'Seminario de Pruebas',      'Sábado',    '17:30:00', '19:30:00'],
      // Laura Gómez
      ...
      ['1007654321', 'Bloque 3 - Piso 1 - Salón 01', 'Bases de Datos',            'Sábado',    '17:30:00', '19:30:00'],
      // Andrés Martínez
      ...
      ['1009876543', 'Bloque 2 - Piso 1 - Salón 01', 'Cálculo Integral',          'Sábado',    '17:30:00', '19:30:00'],
    ];
    ...
    for (let i = 0; i < ests.length; i++) {
      for (let j = 0; j < 3; j++) {
        ...
        await db.query('INSERT IGNORE INTO estudiante_horarios (estudiante_id, horario_id) VALUES (?,?)',
          [ests[i].id, hId]);
      }
    }
    ...
    console.log('╠══════════════════════════════════════════════════════╣');
    console.log('║  ADMIN  admin@uniguajira.edu.co / Admin2024*         ║');
    console.log('╠══════════════════════════════════════════════════════╣');
    ...
    console.log('║    jperez/Est00001  mgomez/Est00002  arojas/Est00003 ║');
    console.log('║    lmartinez/Est00004  dherrera/Est00005             ║');
    console.log('║    klopez/Est00006  fcastro/Est00007  ndiaz/Est00008 ║');
    console.log('║    jmorales/Est00009  cvargas/Est00010               ║');
    console.log('╠══════════════════════════════════════════════════════╣');
    console.log('║  ⭐ SÁBADO 5:30-7:30 PM (Colombia)                  ║');
    console.log('║    Carlos  → Seminario de Pruebas                   ║');
    console.log('║    Laura   → Bases de Datos                         ║');
    console.log('║    Andrés  → Cálculo Integral                       ║');
    console.log('╚══════════════════════════════════════════════════════╝');
