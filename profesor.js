...
// ── Hora Colombia (Intl, funciona en Railway) ────────────────
function ahoraColombia() {
  ...
  // "sábado" → "Sábado", "miércoles" → "Miércoles", etc.
  const raw     = get('weekday'); // ej: "sábado"
  const diaBase = raw.charAt(0).toUpperCase() + raw.slice(1);

  // Mapa para asegurar tildes correctas igual que en la DB
  const MAPA = {
    'Miercoles': 'Miércoles',
    'Miércoles': 'Miércoles',
    'Sabado':    'Sábado',
  ...
  const dia = MAPA[diaBase] ?? diaBase;

  const hora   = parseInt(get('hour'));
  const minuto = parseInt(get('minute'));

  console.log(`⏰ Colombia: ${hora}:${String(minuto).padStart(2,'0')} — ${dia}`);

  return { hora, minuto, dia, minTotal: hora * 60 + minuto };
}
...
router.get('/clases-hoy', verificarToken, soloProfesor, async (req, res) => {
  try {
    const { dia, minTotal } = ahoraColombia();
...
router.post('/registrar-asistencia', verificarToken, soloProfesor, async (req, res) => {
  ...
  try {
    ...
    if (diff > 90)  return res.status(400).json({ error: 'El tiempo para registrar ha expirado' });
